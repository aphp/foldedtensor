...
VERSION_FILE = ROOT / "foldedtensor" / "__init__.py"
...
def normalize_version(version: str) -> str:
    if not re.fullmatch(RELEASE_VERSION_PATTERN, version):
        raise ReleaseError(
            ...
            "Expected a Python release version like 0.5.0 or 0.5.0.dev0."
        )
    return version
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare a release branch, commit, and PR for foldedtensor"
    )
    parser.add_argument(
        "version",
        nargs="?",
        help="Target version, for example 0.5.0 or 0.5.0.dev0",
    )
    parser.add_argument(
        "--bump",
